...
flash: $(OUTPUT_DIRECTORY)/nrf51822_xxaa.hex
	...
	openocd -f interface/stlink-v2.cfg  -f target/nrf51.cfg -c init -c "reset halt;flash write_image erase $<;reset;exit"
...
flash_softdevice:
	...
	#nrfjprog --reset -f nrf51
	openocd -f interface/stlink-v2.cfg  -f target/nrf51.cfg -c init -c "program $(SDK_ROOT)/components/softdevice/s130/hex/s130_nrf51_2.0.1_softdevice.hex verify reset exit"
